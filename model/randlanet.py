...
from model.attention import BertCrossattLayer
...
class RandLANet(BaseModel):
    ...
    **Architecture**
    ...
    def __init__(
        ...
        # TCIL: orthogonal constraint
        self.binary_head = nn.Sequential(
            SharedMLP(dim_feature, 64, activation_fn=nn.LeakyReLU(0.2)),
            SharedMLP(64, 32, activation_fn=nn.LeakyReLU(0.2)), nn.Dropout(0.5),
            SharedMLP(32, 3, bn=False))  # 3 or 7(step=1); 10(step=2)

        # Text-LiDAR interaction
        self.Multimodal_interaction = BertCrossattLayer(hidden_size=512, hidden_dropout_prob=0.1, num_attention_heads=8,
    ...
    def forward(self, inputs):
        ...
        if self.step > 0:
            ...
            if self.step == 1:
                embeddings = torch.load(os.path.join(current_dir,
                    'word/semantickitti/text_embed/semantickitti_12_clip-ViT-B16_text_embed_step_1.pth'))
            elif self.step == 2:
                embeddings = torch.load(os.path.join(current_dir,
                    'word/semantickitti/text_embed/semantickitti_20_clip-ViT-B16_text_embed_step_2.pth'))
            embeddings_ = embeddings.cuda()
            word_embedding = embeddings_.expand(feat.shape[0], -1, -1).clone()
            word_embedding = word_embedding.to(feat.dtype)
            feat_cr = self.Multimodal_interaction(feat.squeeze(-1).permute(0, 2, 1), word_embedding)
            feat = feat_cr.permute(0, 2, 1).unsqueeze(-1)
        ...
        # orthogonal constraint for old/new
        scores_binary = self.binary_head(feat)

        return scores.squeeze(3).transpose(1, 2), inner_feat, feat.squeeze(3), scores_binary.squeeze(3).transpose(1, 2)
