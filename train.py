...
def validate(writer, logger, vset, vloader, epoch, model, device, args):
    ...
    # Initialize metrics
    metric = Metrics(class_names, device=device, mask_unlabeled=True, mask=list(np.arange(1, len(current_step_classes)+1)))
    model.eval()
    with torch.no_grad():
...
def main(args):
    ...
    for e in range(args.epochs):
        ...
        if e % args.eval_every_n_epochs == 0 and e > 0:
            ...
            if miou > best_miou:
                best_miou = miou
                logger.cprint('******************* Best Model Saved, mIoU: %.2f *******************' % (miou))
        ...
        for i, data in enumerate(pbar):
            ...
            # logger record
            logger.cprint(
                "=====Epoch %d/%d, Loss: %.2f, mIoU: %.2f, Progress=====" % (e + 1, args.epochs, l.item(), miou))
    ...
    if miou > best_miou:
        best_miou = miou
        torch.save(model.state_dict(), logdir + "/val_best.pth")
        logger.cprint('******************* Best Model Saved, mIoU: %.2f *******************' % (best_miou))
